...
Este guia fornece padrões e exemplos para refatorar componentes existentes para usar os design tokens da biblioteca `@rainer/design-tokens`.
...
#### ✅ Depois
```tsx
import { BORDER_RADIUS } from '@rainer/design-tokens';
...
import { SEMANTIC_COLORS } from '@rainer/design-tokens';
...
#### ✅ Depois
```tsx
import { CARD } from '@rainer/design-tokens';
<div className={cn(
  CARD.BACKGROUND,
  CARD.BACKDROP,
  CARD.BORDER
...
#### ✅ Depois
```tsx
import { BADGE } from '@rainer/design-tokens';
<span className={cn(
