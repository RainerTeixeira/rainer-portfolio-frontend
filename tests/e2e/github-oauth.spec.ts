/**
 * Teste de Login com GitHub usando Playwright
 *
 * Este teste usa Playwright com configurações anti-detecção
 * para testar o fluxo completo de OAuth do GitHub.
 *
...
test.describe('GitHub OAuth - Fluxo Completo', () => {
  test('Login com GitHub - Fluxo Completo', async ({ page, consoleHelper }) => {
    test.setTimeout(600000); // 10 minutos

    console.log('\n═══════════════════════════════════════════════════════');
    console.log('  🔒 Iniciando teste de OAuth GitHub');
    console.log('═══════════════════════════════════════════════════════\n');
    ...
    await page.screenshot({
      path: 'test-results/github-oauth-step1-login.png',
      fullPage: true,
    });
    ...
    // Aguarda e clica no botão GitHub
    const githubButton = page.locator(
      'button:has-text("GitHub"), button[aria-label*="GitHub" i]'
    );
    await githubButton.waitFor({ timeout: 10000 });
    console.log('✅ Botão GitHub encontrado');

    await githubButton.click();
    console.log('✅ Botão clicado');

    console.log('\n───────────────────────────────────────────────────────');
    console.log('  ETAPA 3: Aguardando redirecionamento para GitHub');
    console.log('───────────────────────────────────────────────────────\n');

    // Aguarda navegação para GitHub/Cognito
    await page
      .waitForURL(
        url =>
    ...
    await page.screenshot({
      path: 'test-results/github-oauth-step3-oauth.png',
      fullPage: true,
    });
    ...
    console.log('  🔑 Digite sua senha do GitHub no navegador');
    ...
    await page.screenshot({
      path: 'test-results/github-oauth-step4-callback.png',
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: 'test-results/github-oauth-step6-final.png',
      fullPage: true,
    });
