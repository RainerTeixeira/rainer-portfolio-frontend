...
describe('bookmarksService', () => {
  ...
  test('createBookmark faz POST e retorna bookmark', async () => {
    ...
    const response = { success: true, data: { id: 'b1', ...payload, createdAt: '', updatedAt: '' } };
  ...
  test('getBookmarksByCollection usa query name', async () => {
    ...
    expect(url.searchParams.get('name')).toBe('Favoritos');
  });
});
