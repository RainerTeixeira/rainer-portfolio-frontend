...
import { ERROR_MESSAGES, FORM_CONFIG, REGEX_PATTERNS } from '@/constants/design-tokens'
...
export function validateUsername(username: string): ValidationResult {
  ...
  if (username.length < FORM_CONFIG.USERNAME_MIN_LENGTH) {
    errors.push(`Username deve ter no mínimo ${FORM_CONFIG.USERNAME_MIN_LENGTH} caracteres`)
  }
  
  if (username.length > FORM_CONFIG.USERNAME_MAX_LENGTH) {
    errors.push(`Username deve ter no máximo ${FORM_CONFIG.USERNAME_MAX_LENGTH} caracteres`)
  }
...
export function validateMessage(message: string): ValidationResult {
  ...
  if (message.length < FORM_CONFIG.MESSAGE_MIN_LENGTH) {
    errors.push(`Mensagem deve ter no mínimo ${FORM_CONFIG.MESSAGE_MIN_LENGTH} caracteres`)
  }
  
  if (message.length > FORM_CONFIG.MESSAGE_MAX_LENGTH) {
    errors.push(`Mensagem deve ter no máximo ${FORM_CONFIG.MESSAGE_MAX_LENGTH} caracteres`)
  }
...
export const contactFormSchema = {
  name: (value: string) => validateUsername(value),
