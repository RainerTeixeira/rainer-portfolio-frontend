...
import type { Category, Post, TiptapJSON } from '@/types/database';
...
export function generateCategoryMetadata(category: Category): Metadata {
  return generateMetadata({
    ...
    description:
      category.description || `Explore artigos sobre ${category.name}`,
...
export function generateArticleStructuredData(post: Post) {
  ...
  return {
    ...
    author: {
      '@type': 'Person',
      fullName: 'Rainer Teixeira', // TODO: Carregar autor com relação
...
export function generateBreadcrumbStructuredData(
  items: { fullName: string; url: string }[]
) {
  const siteUrl =
    process.env.NEXT_PUBLIC_SITE_URL || 'https://rainersoft.com.br';
