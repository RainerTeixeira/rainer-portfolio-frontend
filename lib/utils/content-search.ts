...
export async function searchContent(query: string): Promise<SearchResult[]> {
  ...
  try {
    ...
    const results: SearchResult[] = response.data.map((post: any) => ({
      id: post.id,
      title: post.title,
      slug: post.slug,
      type: 'post' as const,
...
/**
 * Re-export searchContent da biblioteca @rainersoft/utils
 * Migrado para a biblioteca para reutilização em outros projetos
 */
export { searchContent as searchPortfolioContent } from '@rainersoft/utils';
