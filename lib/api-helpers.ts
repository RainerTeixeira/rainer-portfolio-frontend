 ...
 */
export function generateExcerpt(content: TiptapJSON, maxLength: number = 160): string {
 ...
 * @returns DTO pronto para enviar para API
 */
export function preparePostForUpdate(formData: Partial<{
  title: string
  content: TiptapJSON
  categoryId?: string
  status?: PostStatus
 ...
 */
export function validatePostData(data: CreatePostDTO | UpdatePostDTO): string[] {
  const errors: string[] = []
